...
DESCRIPTION="A Flutter based Cwtch UI"

# Homepage, not used by Portage directly but handy for developer reference
HOMEPAGE="https://cwtch.ui"

# Point to any required sources; these will be automatically downloaded by
# Portage
SRC_URI="https://git.openprivacy.ca/cwtch.im/cwtch-ui/releases/download/v${PV}/cwtch-v${PV}.tar.gz"
...
src_install() {
	...
	sed "s|PREFIX|/usr|g" cwtch.template.sh > "${D}/usr/bin/cwtch" || die
	chmod a+x "${D}/usr/bin/cwtch" || die

	mkdir -p "${D}/usr/share/icons" || die
	cp cwtch.png "${D}/usr/share/icons" || die

	mkdir -p "${D}/usr/share/cwtch" || die
	...
	sed "s|PREFIX|/usr|g" cwtch.template.desktop > "${D}/usr/share/applications/cwtch.desktop" || die
	chmod a+x "${D}/usr/share/applications/cwtch.desktop" || die
}
