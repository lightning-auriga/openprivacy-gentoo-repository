...
DESCRIPTION="Automatically generated Cwtch C-Bindings from a high level specification"

# Homepage, not used by Portage directly but handy for developer reference
HOMEPAGE="https://git.openprivacy.ca/cwtch.im/autobindings"
...
# a dependency.
RDEPEND="networkmanager? ( >=net-misc/networkmanager-1.46.0 )
...
src_compile() {
	ego run generate/generate_bindings.go || die
	emake linux || die
}
...
src_install() {
	...
	cp "${WORKDIR}/build/linux/libCwtch.x64.so" "${D}/usr/lib64/cwtch/libCwtch.so" || die
}
